...
def run_eda(full_df):
    ...
    for col in amenities:
        ...
        print(full_df.groupby(col)["price"].mean())

    # wykresy
    for col in amenities:
        plt.figure(figsize=(6, 4))
        full_df.groupby(col)["price"].mean().plot(kind="bar")
        plt.title(f"Average Price by {col}")
        plt.ylabel("Average price")
        plt.show()

    # Sprawdzenie ceny za m^2
    for col in amenities:
        print(f"\nAverage price per m2 by {col}:")
        print(full_df.groupby(col)["price_per_m2"].mean())
